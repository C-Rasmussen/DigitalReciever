...
# breakpoint()
wait = 0
for sym in range(2*Lp, len(correlate)-2*Lp):

    first_arr = correlate[sym:sym+30]
    second_arr = correlate[sym+130:sym+160]
    wait = wait + 1
    
    if first_arr == second_arr and wait>30:
        new_uw = first_arr
        wait = 0
        columns = columns + 1
...
for i in range(len(i_r)):
  ...
  #####redo now with correct phase offset   
  
midpoint = len(theta_arr) // 2
second_half = theta_arr[midpoint:]
Phase_Offset = sum(second_half) / len(second_half)   
...
S = -np.sin(Phase_Offset)
for i in range(len(i_r)):
    i_r[i] = C*i_r[i] - S*q_r[i]
    q_r[i] = S*i_r[i] + C*q_r[i]
...
total_symbols = int((rows)*columns) #+5 accounts for looking at the above 5
